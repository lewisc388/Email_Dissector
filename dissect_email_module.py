...
# -----------------[Process Flags]-------------------
def process_flags(parser, default_output):
  unprocessed_data = []
  sourcefile = ""
  ...
  return unprocessed_data, outputfile
...
def dissect_data(unprocessed_data):
  processed_data = {}
...
# replaced with '?'
def output_formatted(outputfile, processed_data, parser):
  try:
...
if __name__ == "__main__":
  ...
  desc = '''Dissect Email processes Original Email Messages into a usable dictionary format. This allows for easy lookups of flags and the ability to integrate with automations and scripts.'''
  parser = argparse.ArgumentParser(description=desc, exit_on_error=True, add_help=True)

  # ---------------[Define Parser Flags]---------------
  parser.add_argument(
    "-f", "--filename", required=True, dest="sourcefile",
    help="Source filename (.txt) containing Original Email Message"
  )
  parser.add_argument(
    "-o", "--output", dest="outputfile",
    help=f"Output destination (.txt) for the dictionary structure. Default: {default_output}"
  )

  unprocessed_data, outputfile = process_flags(parser, default_output)
  processed_data = dissect_data(unprocessed_data)
  output_formatted(outputfile, processed_data, parser)
